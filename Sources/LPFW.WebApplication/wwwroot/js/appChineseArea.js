/*
*    根据全国三级城市联动 js版 area.js 调整，修改选择控件的 id 和 Name 的命名规则，适应 LPFW 需要。
*/
function Dsy(){
    this.Items = {};
}
...
var s=["AddressVM.ProvinceName","AddressVM.CityName","AddressVM.CountyName"]; // 三个select的name
var opt0 = ["省份", "地级市", "市县区"];
...
/**
 * 在地址相关的 HTMLHelp扩展组件中使用下面 AddressVM_ProvinceName...来过渡数据，在执行 _init_area() 之后在执行下面的方法，基本就是如下的方式：
 * _init_area();
 * _setAreaWithAddressVM();
 * */
function _setAreaWithAddressVM() {
    document.getElementById(s[0]).value = $("#AddressVM_ProvinceName").val();
    change(1);
    document.getElementById(s[1]).value = $("#AddressVM_CityName").val();
    change(2);
    document.getElementById(s[2]).value = $("#AddressVM_CountyName").val();;
